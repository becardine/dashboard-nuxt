<template>
  <div>
    ...
    <TransactionAdd @after-add="afterAdd" v-if="isAdding" @cancel="isAdding = false" />
    ...
    <div class="mt-4">
      <div class="space-y-8">
        ...
        <div v-for="(group, index) in transactionsGrouped" :key="index">
          ...
          <div class="space-y-3">
            <Transaction v-for="transaction in group" :key="transaction.id" :transaction="transaction" @update="onUpdate"/>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    onFilter(filter) {
      this.$store.dispatch('transactions/getTransactions', filter). then((response) => {
